import 'dart:ui';

class ColorManager{
  ColorManager._();
  static const Color kPrimaryColor = Color(0xff04332D);

  static const Color kBlackColor = Color(0xff000000);

  static const Color kGreyColor = Color(0xff505050);
  static const Color kGrey6Color = Color(0xff737373);
  static const Color kWhite2Color = Color(0xffFAFAFA);

  static const Color kLightWhiteColor = Color(0xfff6f6f6);
  static const Color kGrey5Color = Color(0xff999999);
  static const Color kWhiteColor = Color(0xffffffff);
  static const Color kRedColor = Color(0xffFC1B1A);
  static const Color kBoldRedColor = Color(0xff7c0000);
  static const Color kLightGreyColor = Color(0xffEDEDED);
  static const Color kGrey2Color = Color(0xff686F80);
  static const Color kGrey3Color = Color(0xff828282);
  static const Color kGrey4Color = Color(0xff696969);

  static const Color kGreenColor=Color(0xff08A43A);
  static const Color kLightGreenColor=Color(0xff16A99F);
}